...
> > 3. **.autodev/reference/** — immutable specs (workflow-specification.md §4 Guardrail Engine for guardrails)
...
| `ARCHITECTURE.md` | Developer-facing system design: 35 sections covering every component | §6 Guardrails, §7-8 Background + Fallback, §9 Category System |
...
- **Shared mock fixture**: T8 creates `test/mocks/pi-session.ts` exporting `mockCreateAgentSession` (returns a fake session with `subscribe()`, `abort()`, `dispose()`, and controllable event emission) and `mockSessionSubscribe`. This mock is shared — T9 imports it to mock the background manager. Follow the same bun:test mock() pattern as magic-context.ts.
...
- [x] 7. Build guardrail engine via tool_call interception
  What to do: NOTE: T7/T8/T9 replace ONLY the body of the existing `register()` function in `guardrails/index.ts`, `background/index.ts`, `delegation/index.ts` respectively. Do NOT modify `extensions/autodev/index.ts` — imports and registration order are already correct from T5. The `register` signature stays `(pi: ExtensionAPI) => void` (sync registration of async handlers). Implement AutoDev's hard stops as pi `tool_call` event handlers that block violating actions. Register a `tool_call` event listener in the AutoDev extension that inspects every tool call and blocks if a hard stop is violated. Hard stops: (1) never-deploy-directly — block direct deploy actions by any agent other than the Navigator (action type check); (2) no-secrets-in-code — block `write`/`edit` calls that contain API keys, tokens, passwords (regex check); (3) evidence-or-it-didnt-happen — block `bash` calls that run `git commit` if no evidence file exists in `.omo/evidence/` for the current task. NOTE: AGENTS.md Standing Order 5 references `.autodev/evidence/` but this is STALE from the pre-pi-foundation era. ARCHITECTURE.md §6 and the actual evidence directory are at `.omo/evidence/`. Use `.omo/evidence/` as the canonical path.; (4) one-task-at-a-time — block new task creation if a task is already in progress. State tracked via `.autodev/active-task.json` (T7 creates this file itself: {task_id, started_at}). Does NOT depend on T13 or T16.; (5) follow-the-plan — HARD STOP (block). Block implementation that deviates from the active plan in .omo/plans/. Detection: compare write/edit target paths against files listed in the active plan's scope. If a target file is NOT mentioned in the plan, block with reason 'follow-the-plan'. This is consistent with guardrails.yaml (enforcement: block_action) and workflow-specification.md §4.; (6) ci-is-the-hard-gate — block `bash` calls that run `gh pr merge` if CI is not green. Also implement soft stops (warnings): suggest-review, warn-scope, flag-missing-evidence. T7 builds a minimal expression evaluator for the guardrail DSL in guardrails.yaml. The evaluator supports: `==`, `!=`, `>`, `AND`, `NOT`, `path_starts_with(...)`, `contains_secrets(...)`, `evidence_exists`, `active_tasks`, `action_type`, `agent`, `ci_status`. The evaluator operates on a GuardrailContext derived from the tool_call event. T7 must implement ALL hard stops present in guardrails.yaml, not just the 6 named above. The YAML contains 10 hard stops: the 6 listed plus never-approve-own-work (block self-review), never-modify-reference-docs (block write to .autodev/reference/), never-modify-debate-transcripts (block write to .autodev/debates/). Add acceptance tests for these 3 additional stops. Write tests: plant a secret in a write call → blocked; write evidence then commit → allowed; merge without CI → blocked.
  ...
  References: Pi tool_call event: `pi.on("tool_call", async (event, ctx) => { if (violates) return { block: true, reason: "..." } })`. Hard stops from .autodev/reference/workflow-specification.md §4 Guardrail Engine. Guardrail config at .autodev/config/guardrails.yaml. Pi docs: https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/extensions.md.
...
- [x] 8. Build background agent management and model fallback
  What to do: Build two systems: (a) Background agent manager — spawn subagent sessions via `createAgentSession()` with `SessionManager.inMemory()`. Track task lifecycle (pending→running→completed/error/cancelled). Concurrency control (max 5 per provider key — all sessions to the same LLM provider share one 5-slot pool. Config schema: T8 creates `.autodev/config/concurrency.yaml` with `{ ollama-cloud: { max: 5 } }`. Default 5 when unspecified. Since 10 of 13 agents use ollama-cloud, the entire crew is capped at 5 concurrent background tasks to that provider — this is intentional to prevent rate limit hits.). Poll completion via `session.subscribe()` events (listen for agent_end, message_end). Circuit breaker (sliding window — trips when no events received for 180s since the LAST event, not since session start. Configurable via `staleTimeoutMs` parameter. On trip: abort session, mark task error, notify parent with reason 'circuit-breaker-timeout'. If a final event arrives concurrently with trip, the event wins — idempotent notify, no double-abort.). Parent-wake notifier (notify parent session when child completes). Error classifier (retryable vs fatal). (b) Model fallback — when a model call fails (429, 500, 502, 503, 504, timeout), extract error info from session events, resolve the agent's `fallback_models` chain from config, abort current session, re-prompt with fallback model. Support both proactive (configured per agent) and reactive (auto-switch on API errors) fallback. Config in .autodev/config/ or .pi/settings.json.
...
- [x] 9. Build category system and task delegation
  What to do: Build the task delegation system. Built-in categories: quick (trivial fixes), deep (autonomous problem-solving), ultrabrain (hard logic), visual-engineering (frontend/UI), artistry (creative), writing (docs/prose), unspecified-low (general low-effort), unspecified-high (general high-effort). Category model assignments loaded from config (.autodev/config/ or .pi/settings.json) — NOT hardcoded. Default model mapping: quick=glm-5.2:cloud, deep=deepseek-v4-pro, ultrabrain=deepseek-v4-pro (with thinkingLevel="xhigh" — this is a createAgentSession() parameter, NOT a model string suffix. Verify against pi SDK docs (docs-corpus/pi/sdk.md) before implementing. If the SDK doesn't support thinkingLevel, T9 logs a warning and proceeds without it — it's a capability enhancement, not a hard requirement. Do NOT encode it in the model string.), visual-engineering=glm-5.2:cloud, artistry=glm-5.2:cloud, writing=glm-5.2:cloud, unspecified-low=glm-5.2:cloud, unspecified-high=glm-5.2:cloud. All model strings must be validated against the static allowlist at `.autodev/config/models.json` (created in T3 of Plan 1). No real API calls during development — the installer (T19) verifies model availability at deployment. Custom categories configurable in .autodev/config/. Register a `task` pi tool via `defineTool()` that accepts either `category` or `subagent_type` (mutually exclusive). When category is given: spawn a background session (via T8's manager) with the category's model and a system prompt that includes the task description + skill context. When subagent_type is given: spawn a specific crew agent (explore, oracle, etc. from T4's 13 agents). Support `run_in_background: true` for async execution. Support `load_skills` as an OPTIONAL parameter (`load_skills?: readonly string[]`, default `[]`). T9 accepts it, type-validates as string array, and silently ignores the contents — the spawned session's system prompt contains no skill context. No validation against a skill registry. No error on non-empty input. Add a `// TODO(T12): inject skill prompts` comment. This is a deliberate no-op, not an error path.
...
- [x] F1. Verify scope — 6 hard stops, background manager, category system
- [x] F2. Code quality — guardrail logic, session lifecycle, model fallback
- [x] F3. Manual QA — mock tests for each hard stop, concurrent session test, fallback test, category routing test

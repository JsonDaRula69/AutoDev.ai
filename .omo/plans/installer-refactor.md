...
- [ ] 1. Create `install.sh` standalone shell bootstrap script at repo root
...
- [ ] 2. Remove dead lifecycle hooks from `package.json`, repoint `bin` to `scripts/cli.ts`, delete guard/postinstall scripts
...
- [ ] 3. Create internal install module (`extensions/autodev/installer/install-module.ts`) — no secrets
...
- [ ] 4. Create config module (`extensions/autodev/installer/config-module.ts`) — all secrets, interactive-only, sub-commands
...
- [ ] 5. Refactor doctor (`extensions/autodev/installer/doctor.ts`) to trigger install + config modules
...
- [ ] 6. Expand `scripts/cli.ts` with full sub-command routing, rewrite `orchestrator/cli.ts` dispatcher
...
- [ ] 8. Dead-code sweep — delete `index.ts`, confirm zero references to removed symbols
...
- [ ] F1. Plan compliance audit — verify every todo's acceptance criteria are met and every "Must NOT do" is respected. Check no project-level steps snuck in.
- [ ] F2. Code quality review — no dead code, no duplicated logic between install-module and config-module, no orphaned imports, no `any` types introduced, no `@ts-nocheck`. Verify `readEnv` and `setEnvVars` both accept optional `envPath` parameter (symmetric API). Verify `StateScope` includes `"config"` scope.
- [ ] F3. Agent-executable integration QA — run `install.sh` in a temp dir with `PATH` restricted to verify the missing-bun branch (auto-install attempted, PATH export works); run `install.sh` with a stubbed `autodev` on PATH to verify the doctor handoff; run `autodev config` with a `MockPrompter`-injected deps to verify secret entry writes to `~/.pi/agent/.env` and `~/.pi/agent/auth.json`; run `autodev doctor` twice (once to trigger fixes + config, once to confirm all-green with mocked env — verify doctor re-runs health checks after module triggers). All outputs recorded as evidence. No human gate.
- [ ] F4. Scope fidelity — confirm the refactor did not touch agent definitions, guardrails, or any module outside `installer/`, `orchestrator/cli.ts`, `scripts/`, and `extensions/autodev/index.ts` (the ONLY allowed change to index.ts is adding `.env` loading at the top of `register()`).

...
Alert the liaison that a PR has been merged and CI is green. Post in the liaison communication channel (Discord, Slack, or the configured crew channel) that the PR is merged and ready for deployment.

If a webhook endpoint is configured (in `.pi/settings.json` under the AutoDev deployment config), POST the deployment signal:

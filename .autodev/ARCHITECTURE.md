...
10. Human reviews and merges PR
...
- **All PRs require human review before merge.** AutoDev never deploys directly.
- **Human override:** Any human comment with `@autodev hold` on a PR prevents merge until explicitly released with `@autodev proceed`.
